...
const user = {
    ...
    create_user: async function (req, res) {
        ...
        if (profilePhoto)
            fs.unlinkSync(path.join(__dirname, "../public/uploads/", profilePhoto));
        ...
        if (err) {
            if (profilePhoto)
            fs.unlinkSync(
                path.join(__dirname, "../public/uploads/", profilePhoto)
            );
            req.flash("error", "Server error.");
            return res.redirect("/register");
        }
        ...
        if (results.length > 0) {
            ...
            req.flash("error", "Credential already registered.");
            return res.redirect("/register");
        }
        });
